...
def action_on_gt_threshold(**kwargs) -> None:
    ...
    """
    engine = get_engine()
    id = kwargs['ti'].xcom_pull(key='job_ID', task_ids=['insert_recs'])

    with engine.begin() as connection:
        connection.execute(text(f"""
            BEGIN; 
                UPDATE dummy_job SET is_active = FALSE 
                WHERE id = {id[0]}; 
                
                UPDATE dummy_job_result SET is_successful = TRUE, is_gt_th = TRUE 
                WHERE job_id = {id[0]}; 
            COMMIT;
        """))
    ...
    with engine.begin() as connection:
        connection.execute(text(f"""
            BEGIN; 
                ...
                UPDATE dummy_job_result SET is_successful = TRUE, is_gt_th = FALSE 
                WHERE job_id = {id[0]}; 
            COMMIT;
        """))
    ...
    with engine.begin() as connection:
        connection.execute(text(f"""
            BEGIN; 
                ...
                UPDATE dummy_job_result SET is_successful = FALSE 
                WHERE job_id = {id[0]}; 
            COMMIT;
        """))
...
with DAG(
    ...
    description = 'Candidate DAG',
    schedule_interval = "* * * * *",#every minute
    start_date = days_ago(0),#now
    catchup = False,
    tags = ['EQ_WORKS'],
) as dag:
    ...
    #   Your code here.
    task_create_tables >> task_insert_recs >> task_get_hit_count >> [task_action_on_error, task_branching]
